...
function showlist() {
  ...
  for (var i = 0; i < shoplist.list.length; i++) {
    ...
    var new_item_list = item_list.replace("{{1}}", i + 1).replace("{{吹風機}}", item.name).replace("{{300}}", item.price).replace("{{deid}}", del_item_id).replace("{{del_item_id}}", i).replace("{{id}}", item_id);
  ...
  var new_total_item_price = total_item_price.replace("{{money}}", total_price);
